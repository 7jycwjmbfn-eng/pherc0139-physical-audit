...
`material` sits inside `valid`, and `centerline` and `boundary_poor` inside `material`, with zero violations. `recto_band` is dilated once by construction, so 28 to 30 percent of it falls just outside `material`; that is intended, not a packing error.
