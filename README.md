...
- `labels1203_L1.zarr` (497 MB): window 2016 x 3456 x 3456 at origin (3936, 0, 0). Same bits plus **boundary_poor (16)**: material whose gap visibility falls below 0.40, meaning no resolvable air boundary within 57 um at the truth resolution. It covers **72.8 percent of the material voxels** in this volume, which is a statement about how fused this scroll is rather than a small flagged subset, so size any stratification on it accordingly. That map also bears on the official 2027 question of telling "no ink" from "ink not yet recovered"; in boundary-poor tissue the limiting factor is physics, not the pipeline.
