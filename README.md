...
- **PHerc1203** (on the 13-scroll Grand Prize list; heavily compressed, with 7-14 percent of its tissue windows unmeasurable by structure-tensor spacing, most of them clustered, and fused blocks confirmed visually at 4.8 um): 9.362 um full scroll + 2.403 um scan covering the full cross-section over 36 mm. Registration held-out error **2.4 um** median. Note that the `boundary_poor` bit plane in the shipped labels is a broader map than this 7-14 percent figure; see the dataset section.
...
2. **Side-of-sheet placement** (`side_inward` in the evaluator, not to be confused with `recto_side_ratio`, which is a separate mass-overlap quantity). m7 is documented as a recto (inward-facing surface) predictor. We test this directly: at each centerline point carrying enough local structure to define a normal, sample a band's distance field 2 voxels to each side along that normal (structure tensor, oriented inward by the radial sign; the estimator passes a synthetic-stripe self-test at 4 angles) and count which side is closer.

   The point populations differ by arm, and the conditioning is worth stating plainly. The m7 row keeps coherent points with the prediction within 3 voxels (31.9M points, 11.0 percent of them ties, so 28.4M decided). The null and ideal rows keep the subset of *those* points that also has their own band within 3 voxels: 21.6M and 30.2M points respectively. So the calibration below is a within-real comparison, not three independently drawn samples. The evaluator emits both readings, the conditioned one and a `_full` variant that selects each control from the whole coherent centerline set; `side_inward` itself is identical under either, since the real arm is the same set both ways (`code/test_side_arms_equivalence.py` holds the pre-change implementation next to the shipped one and checks that; the decided counts in the committed outputs are also unchanged across the two runs). Measured, the conditioning costs little: on PHerc0139 the null is 0.5011 under both readings and the ceiling moves 0.9270 to 0.9241, taking the skill from 0.4459 to 0.4490; on PHerc1203, 0.5011 to 0.5012 and 0.8835 to 0.8797, skill 0.1286 to 0.1296.
   ...
   The standalone evaluator reimplements this instrument from the packaged labels alone and returns 0.6911 / 0.5011 / 0.9270 with a skill of 0.4459, against 0.691154 / 0.501147 / 0.927155 here. It takes sheet normals from the smoothed material mask, because the label package does not carry the 1.129 um grayscale the pass above used. That substitution turns out to cost almost nothing: 28,420,703 decided cases against 28,393,954 here, a difference of 0.09 percent, with the inward fraction unmoved in the fourth decimal.
...
- **Difficult tissue confirmed and mapped**: 7-14 percent of tissue windows are unmeasurable by structure-tensor spacing at 19 um (73-86 percent of them clustered, growing along z); a 4.8 um look inside the largest cluster shows crosshatched fiber bundles pressed into near-solid blocks (1 mm windows at 99.75 percent material). A second, threshold-robust zone variable, gap visibility (the fraction of local material within 57 um of a resolvable air boundary), reads about 0.87 in loose tissue against 0.28-0.35 in the difficult zones, and ships as a bit plane in the label volume. The two variables have very different reach and the difference matters for anyone stratifying on them: the unmeasurable-window figure is 7-14 percent, but gap visibility below 0.40 covers **74.6 percent of the material voxels in this scroll**, because the fused zone is itself 63-73 percent of the tissue at this threshold (`results/1203/calib1.json`). The `boundary_poor` bit is a map of the majority of this scroll, not of a small difficult minority.
...
- **Side-of-sheet replication** (`side_inward`): 9.07M points, m7 inward fraction 54.84 percent against a 50.11 null and an 88.31 ideal ceiling. That is 12 percent of the ideal signal, versus 45 percent on PHerc0139. The standalone evaluator returns 0.5502 / 0.5011 / 0.8835 with a skill of 0.1286. The null lands on the same four decimals; the 0.18 point gap in the inward fraction is a sampling difference, since this pass scored every eighth slice and the evaluator scores every fourth, which puts it on twice the points (16.15M decided against 9.07M points here).
...
- `labels1203_L1.zarr` (497 MB): window 2016 x 3456 x 3456 at origin (3936, 0, 0). Same bits plus **boundary_poor (16)**: material whose gap visibility falls below 0.40, meaning no resolvable air boundary within 57 um at the truth resolution.

How much of each volume each bit actually covers, counted over the full volumes (`results/bit_census.json`):

| | PHerc0139 | PHerc1203 |
|---|---|---|
| `valid`, of the window | 42.04% | 20.85% |
| `material`, of valid | 49.78% | 82.16% |
| `centerline`, of material | 10.36% | 2.46% |
| `recto_band`, of material | 51.62% | 13.77% |
| `boundary_poor`, of material | not present | **74.62%** |

Three of those numbers say the same thing about PHerc1203 and are worth reading together. `boundary_poor` is the majority of the material rather than a flagged minority, so stratifying on it does not split this scroll into difficult and easy halves; it is a measurement of how fused the scroll is. The other two follow from that: `recto_band` needs air 1.5 voxels inward of material to exist at all, and per-sheet centerlines need sheets that are still separable, so both collapse in fused tissue. The ideal-recto ceiling quoted for PHerc1203 (88.3) is therefore computed on a band covering 13.77 percent of material, against 51.62 percent on PHerc0139.

`material` sits inside `valid`, and `centerline` and `boundary_poor` inside `material`, with zero violations. `recto_band` is dilated once by construction, so 29.9 percent of it on PHerc0139 and 27.6 percent on PHerc1203 falls just outside `material`; that is intended, not a packing error.

The `boundary_poor` map also bears on the official 2027 question of telling "no ink" from "ink not yet recovered"; in boundary-poor tissue the limiting factor is physics, not the pipeline.
...
Validation: its full output against the published m7 volume is committed at `results/eval_selfcheck_0139.json` and `results/1203/eval_selfcheck_1203.json`, so a rerun can be diffed. The point and arc numbers match this report to four decimals, as does the side null; the inward fraction and ceiling agree to three (0.6911 / 0.5011 / 0.9270 against 0.691154 / 0.501147 / 0.927155). The evaluator takes its normals off the material mask because the label package does not carry the high-resolution grayscale, which moves the decided count by 0.09 percent and nothing else. villa issue #193 asked for objective labels that do not depend on a model; these two volumes are built that way, and the PHerc1203 one covers compressed and boundary-poor tissue.
...
- [Independent thickness zoning](https://github.com/Jinhojeong/vesuvius-surface-geometry-diagnostic/tree/main/results/thickness1203) (Jinhojeong, 2026-08-10): 24-ray run-thickness sampling on the shipped material and centerline bits, no hand-drawn zones, pinned to the release tarball by SHA-256. Its `fused_share` (thick run crossing exactly one centerline segment) is sparse where `boundary_poor` is broad, Spearman 0.23 between them, so the two variables carry different information and TAUIL-Abd-Elilah's protocol uses them as separate strata. Note when comparing scales: `fused_share` flags 10.2 percent of runs overall (median cell 8.3 percent), while `boundary_poor` covers 74.6 percent of material voxels (see the dataset section).
...
- `code/` contains the full pipeline for both scrolls: pass1 (global registration) through pass10 (dataset packaging) for PHerc0139, and the `*_1203.py` drivers that re-run the same machinery on PHerc1203; each stage has a built-in self-test, null control, or ceiling calibration; `eval_surface_pred.py`; the chunk fetcher.
  ...
  One difference between the evaluator and those audit scripts is worth stating, since it was raised on the villa PR by TAUIL-Abd-Elilah. Both used to skip a slice whose prediction held fewer than 100 voxels, which would take that slice's truth out of the denominator along with its failures and so flatter a sparse prediction. `eval_surface_pred.py` is the entry point other people run, so its guard is now truth-only and empty bands get an infinite distance field; `code/test_empty_prediction_denominator.py` pins that, and fails on five assertions against the pre-fix version. The eleven audit scripts still carry the original guard, because they are the record of the run that produced the numbers below rather than a harness for other predictions. Whether that mattered is measured rather than argued: `code/guard_probe.py` counts, slice by slice, how often the guard could have fired on the published m7 volumes. It never did, on either scroll or either form of the guard, and the sparsest scored slice carries 15,931 predicted voxels on PHerc0139 and 738,200 on PHerc1203 against a threshold of 100 (`results/guard_probe.json`). Every number in this report is unaffected.
