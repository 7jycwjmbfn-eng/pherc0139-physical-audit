...
def slab_work(iz):
    ...
    for k in range(0, out_shape[0], 4):
        ...
        if tb.sum() < 500 or pb.sum() < 100:
            continue
        # zone map: tile-mean gap visibility = fraction of local material
        # with a resolvable air boundary within 3 vox (57 um). TH-robust
        # (see calib1.json), physically interpretable.
        dt = ndi.distance_transform_edt(tb)
        num = ndi.uniform_filter((tb & (dt < 3)).astype(np.float32), TILE)
        den = ndi.uniform_filter(tb.astype(np.float32), TILE)
        ...
        nd = dnull[ys, xs]
        zf = gv[ys, xs]
        zone = np.where(zf < 0.40, 2, np.where(zf < 0.80, 1, 0))
...
def main():
    ...
    stats["zone_def"] = ("tile-mean gap visibility (material within 57um "
                         "of air): L>=0.8, C 0.4-0.8, F<0.4")
    json.dump(stats, open(OUT / "pass5_stratified_v2.json", "w"), indent=1)
