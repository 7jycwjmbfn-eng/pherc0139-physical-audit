...
def slab_work(iz):
    ...
    for k in range(pred.shape[0]):
        ...
        part["cd_hist"] += np.bincount(
            np.clip(cd.astype(int), 0, 20), minlength=21)
        for r in (1, 2, 3):
...
def main():
    ...
    acc = dict(n_center=0, hits={r: 0 for r in R}, null_hits=0,
               null_hits_r={r: 0 for r in R},
               n_pred=0, pred_far2=0, pred_far4=0,
               hit_int_sum=0.0, hit_int_n=0, miss_int_sum=0.0, miss_int_n=0,
               dist_hist=np.zeros(21, np.int64),
               cd_hist=np.zeros(21, np.int64), per_slab=[])
    with ProcessPoolExecutor(max_workers=4) as ex:
        for part in ex.map(slab_work, list(ZCH)):
            for r in R:
                acc["hits"][r] += part["hits"][r]
                acc["null_hits_r"][r] += part["null_hits_r"][r]
            ...
            acc["cd_hist"] += np.asarray(part["cd_hist"])
    ...
    stats = dict(
        ...
        null_recall_19um=acc["null_hits_r"][1] / n,
        null_recall_37um=acc["null_hits_r"][2] / n,
        null_recall_56um=acc["null_hits_r"][3] / n,
        cd_hist=[int(v) for v in acc["cd_hist"]],
    ...
    print(json.dumps({k: v for k, v in stats.items() if k != "per_slab"},
                     indent=1), flush=True)
    json.dump(stats, open(OUT / "pass5b_stats.json", "w"), indent=1)
    np.save(OUT / "pass5b_dist_hist.npy", acc["dist_hist"])
